<script>
  ...
  import { selected, tool, zoom, minZoom, maxZoom, activeArtboard, objectListVisible, objectSettingsVisible, mapSettingsVisible } from 'Editor/store/ui'
  ...
  subscribers.push(
    tool.subscribe(() => {
      if ($tool === consts.TOOL_SPOT || $tool === consts.TOOL_RECT || $tool === consts.TOOL_OVAL || $tool === consts.TOOL_POLY || $tool === consts.TOOL_TEXT) setters.setSelection([])
  ...
  function handleMouseDown(e) {
    ...
    if (areCoordsInsideCanvas(coordsCanvasInitial) && e.target.closest(`[data-object-type="${consts.CANVAS_OBJECT_ROOT}"]`)) {
  ...
  function zoomOut({ x, y }) {
    ...
    targetOffset = {
      x: canvasRect.x - (distanceFromFocalPointToCanvasCenter.x - distanceFromFocalPointToCanvasCenter.x / zoomMultiplier),
      y: canvasRect.y - (distanceFromFocalPointToCanvasCenter.y - distanceFromFocalPointToCanvasCenter.y / zoomMultiplier),
    }
  ...
  function tick() {
    ...
    canvasRect.width = utilities.easeInOutCubic(elapsed, canvasSize.width * initialZoom, canvasSize.width * $zoom - canvasSize.width * initialZoom, duration)
    canvasRect.height = utilities.easeInOutCubic(elapsed, canvasSize.height * initialZoom, canvasSize.height * $zoom - canvasSize.height * initialZoom, duration)
  ...
  function shouldStartDrawingSpot() {
    if (eventStartedInCanvas && $tool === consts.TOOL_SPOT && areCoordsInsideCanvas(coordsCanvas) && !didStartDrawingPoly) return true
    return false
  }
  ...
  function shouldStartDrawingRect() {
    if (eventStartedInCanvas && $tool === consts.TOOL_RECT && (Math.abs(coords.x - coordsInitial.x) > 3 || Math.abs(coords.y - coordsInitial.y) > 3) && !didStartDrawingPoly) return true
    return false
  }
  function drawRect() {
  ...
  function shouldStartDrawingOval() {
    if (eventStartedInCanvas && $tool === consts.TOOL_OVAL && (Math.abs(coords.x - coordsInitial.x) > 3 || Math.abs(coords.y - coordsInitial.y) > 3) && !didStartDrawingPoly) return true
    return false
  }
  function drawOval() {
  ...
  function shouldPlacePolyPoint(e) {
    if (didStartDrawingPoly && eventStartedInCanvas && e.target.dataset.objectType !== consts.CANVAS_OBJECT_FIRST_POLY_POINT && $tool === consts.TOOL_POLY) return true
    return false
  }
  ...
  function redrawTempPoly(_coords) {
    ...
    tempPolySVG.setAttributeNS(null, 'viewBox', `0 0 ${getCanvasBoundingRect().width} ${getCanvasBoundingRect().height}`)
  ...
  function shouldStartDrawingText() {
    if (eventStartedInCanvas && $tool === consts.TOOL_TEXT && areCoordsInsideCanvas(coordsCanvas) && !didStartDrawingPoly) return true
    return false
  }
