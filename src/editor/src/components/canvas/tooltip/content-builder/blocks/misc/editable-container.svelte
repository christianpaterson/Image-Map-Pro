<script>
  ...
  export function setStrong({ range = getSelectionRange(window.getSelection()) }) {
    if (range.end > range.start) {
      ...
      for (let i = 0; i < model.length; i++) {
        if (i >= range.start && i < range.end && !model[i].bold) newFlag = true
      }
      for (let i = 0; i < model.length; i++) {
        if (i >= range.start && i < range.end) model[i].bold = newFlag
      }
  ...
  // Text edit functions
  function buildTextStyleModel(text) {
    let flagBold = false
    let flagEm = false
    let flagA = false
    let href = ''
...
<div class="relative flex {hiddenClass}">
  <div bind:this={root} contenteditable="true" {id} class="{classes} {editableDivHiddenClass} relative z-10 flex-1 cursor-text" style="{style} outline: 0px solid transparent" bind:innerHTML={text} on:focusout={onFocusOut} on:input={onInput} />
  ...
  <div class="{textareaHiddenClass} flex-1" {style}>
    <FormControl bind:value={text} {onChange} type="textarea" classes="p-0 w-full text-theme-800 dark:text-white" hidden={visualMode} />
  </div>
</div>
