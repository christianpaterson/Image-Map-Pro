...
<div>
  <div class="p-2">
    ...
    <FormControl type="checkbox" name="Responsive" group="general" prop="responsive" description="If checked, the image map will fit its parent element and keep its aspect ratio." />
  </div>
  ...
  <FormSection title="Objects" collapsed opacity="0.5">
    <FormControl type="button-group" label="Pageload Animation" group="objectConfig" prop="pageload_animation" options={pageloadAnimationOptions} />
    ...
    {#if mapSettings.objectConfig.glowing_objects}
      <div transition:fade|local={{ duration: 150 }}>
        <div transition:slide|local={{ duration: 150 }}>
          <FormControl type="checkbox" name="Stop Glowing on Mouseover" group="objectConfig" prop="stop_glowing_on_mouseover" />
          <div class="flex">
            <div class="flex-[3]"><FormControl type="slider" label="Glow Opacity" group="objectConfig" prop="glow_opacity" options={{ min: 0, max: 1 }} /></div>
            <div class="flex-1"><FormControl type="color" label="Color" group="objectConfig" prop="glowing_objects_color" /></div>
          </div>
        </div>
      </div>
    {/if}
    <FormControl type="checkbox" name="Scale Spots on Zoom" group="objectConfig" prop="scale_spots" description="When the image map is zoomed, the spots will keep their specified size in pixels." />
  </FormSection>
  ...
  <FormSection title="Tooltips" collapsed opacity="0.5">
    ...
    {#if mapSettings.tooltips.enable_tooltips}
      <div transition:fade|local={{ duration: 150 }}>
        <div transition:slide|local={{ duration: 150 }}>
          <FormControl type="button-group" label="Tooltip Show Condition" group="tooltips" prop="show_tooltips" options={showTooltipsOptions} />
          <FormControl type="button-group" label="Tooltip Animation" group="tooltips" prop="tooltip_animation" options={tooltipAnimationOptions} />
          <FormControl type="button-group" label="Show Fullscreen Tooltips" group="tooltips" prop="fullscreen_tooltips" options={fullscreenTooltipsOptions} />
          {#if mapSettings.tooltips.fullscreen_tooltips === 'always' || mapSettings.tooltips.fullscreen_tooltips === 'mobile-only'}
            <div transition:fade|local={{ duration: 150 }}>
              <div transition:slide|local={{ duration: 150 }}>
                ...
                <FormControl type="slider" label="Fullscreen Background Opacity" group="tooltips" prop="fullscreen_background_opacity" options={fullscreenTooltipsBackgroundOpacityOptions} />
              </div>
            </div>
          {/if}
          {#if mapSettings.tooltips.fullscreen_tooltips === 'none' || mapSettings.tooltips.fullscreen_tooltips === 'mobile-only'}
            <div transition:fade|local={{ duration: 150 }}>
              <div transition:slide|local={{ duration: 150 }}>
                <FormControl type="checkbox" name="Sticky Tooltips" group="tooltips" prop="sticky_tooltips" description="Tooltips will stick to the mouse. Disabled on touchscreen devices." />
                <FormControl type="checkbox" name="Constrain Tooltips" group="tooltips" prop="constrain_tooltips" description="If a tooltip is too big, it will not go out of the screen bounds." />
              </div>
            </div>
          {/if}
        </div>
  ...
  <FormSection title="Zooming" collapsed opacity="0.5">
    ...
    {#if mapSettings.zooming.enable_zooming}
      <div transition:fade|local={{ duration: 150 }}>
        <div transition:slide|local={{ duration: 150 }}>
          <FormControl type="slider" integer label="Max Zoom" group="zooming" prop="max_zoom" options={{ min: 2, max: 6 }} />
          <FormControl type="checkbox" name="Limit Max Zoom to Image Size" group="zooming" prop="limit_max_zoom_to_image_size" />
  ...
  <FormSection title="Object List" collapsed opacity="0.5">
    ...
    {#if mapSettings.object_list.enable_object_list}
      <div transition:fade|local={{ duration: 150 }}>
        <div transition:slide|local={{ duration: 150 }}>
          <FormControl type="button-group" label="Menu Style" group="object_list" prop="menu_style" options={objectListMenuStyleOptions} />
          ...
          {#if mapSettings.object_list.menu_style !== 'detached'}
            <div transition:fade|local={{ duration: 150 }}>
              <div transition:slide|local={{ duration: 150 }}>
                <FormControl type="button-group" label="Menu Position" group="object_list" prop="menu_position" options={menuPositionOptions} />
              </div>
            </div>
          {/if}
          ...
          {#if !mapSettings.object_list.group_by_artboard}
            <div transition:fade|local={{ duration: 150 }}>
              <div transition:slide|local={{ duration: 150 }}>
                <FormControl type="checkbox" name="Show Only Objects from Active Artboard" group="object_list" prop="show_only_objects_from_active_artboard" />
              </div>
            </div>
          {/if}
