...
<div class="">
  <div class="flex w-full">
    <!-- Slider -->
    <div class="flex items-center w-full relative cursor-pointer mr-2" style="height: 24px;" on:mousedown={handleMouseDown} data-ui-type="track">
      <!-- Track -->
        ...
        <div class="relative w-full" bind:this={trackEl}>
          <div bind:this={ballEl} class="absolute top-1/2 w-0 h-0">
            <div
              ...
              style="width: {ballWidth}px; height: {ballWidth}px; margin-top: {-ballWidth / 2}px; margin-left: {-ballWidth / 2}px"
            />
          </div>
        </div>
      </div>
    ...
    <div style="width: {inputWidth}px;" class="flex-shrink-0">
      <FormControl type="number" bind:value {integer} nopadding {options} />
    </div>
  </div>
</div>
